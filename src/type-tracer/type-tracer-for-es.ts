...
export function buildExpressionTypeProvider(
  ...
  if (!result) {
    cache.set(key, (result = buildExpressionTypeProviderImpl(sourceCode)));
  }
  return result;
}
...
function buildExpressionTypeProviderImpl(
  ...
  const GET_TYPE_INFOS = {
    ArrayExpression: (): TypeInfo => ({ type: "Array" }),
    ...
    Literal: (node: TSESTree.Identifier): TypeInfo => ({
      type: getSimpleExpressionType(node)!,
    }),
    TemplateLiteral: (): TypeInfo => ({ type: "String" }),
    Identifier: getIdentifierTypeInfo,
    ImportExpression: (): TypeInfo => ({ type: "Promise" }),
    ...
    UpdateExpression: (): TypeInfo => ({ type: "Number" }),
    ClassExpression: (): TypeInfo => ({ type: "Function" }),
